...
var _default = (template, envVars, portOffset, basePath, refOverrides = {}, portIncrement = 1, baseImageRepo = 'lambci/lambda') => {
  ...
  .filter(([_, resource]) => isServerlessFunction(resource)).flatMap(([name, resource]) => Object.values(resource.Properties?.Events ?? {}).filter(isApiEvent).map((ApiEvent, i, apiEvents) => [name + (apiEvents.length > 1 ? `_${i}` : ''), {
    ApiEvent,
    functionName: name,
    ...resource
    ...
    const environment = mapValues(resolveEnvValue(refOverrides), { ...functionGlobals.Environment?.Variables,
