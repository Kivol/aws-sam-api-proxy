...
_commander.program.command('start <apiName>').description('start an api').requiredOption('-p, --port <port>', 'The port the server will run on').option('-n, --env-vars <envVars>', 'JSON file containing values for Lambda function\'s environment variables.').option('-t, --template <template>', 'Relative path to the SAM template', 'template.yaml').option('--docker-network <dockerNetwork>', 'The docker network you want your containers to connect to').option('--base-path <basePath>', 'The base path of the API', process.cwd()).option('--ref-overrides <refOverrides>', 'Comma-separated key=value pairs to use when resolving Ref calls in your function environments').option('--port-increment <portIncrement>', 'The increment value for ports that containers will use', '1').option('--log-level <logLevel>', 'The log level to use (trace, debug, info, warn, error)', 'debug').option('--base-image-repo <baseRepo>', 'Repository to pull base container image(eg. public.ecr.aws/p0o6c8z6/lambda), defaults to lambci/lambda from system default (usually docker.io)', 'lambci/lambda').option('--skip-pull-images', 'Optionally skip to pull base image').action(async (apiName, options) => {
